...
if [[ (-n "${SNAPSHOT_PRODUCT}") &&
    ...
    if [[ (-n "${SNAPSHOT_DEPLOYMENT_UNIT}" ) ]]; then
        SNAPSHOT_REPO="${SNAPSHOT_REPO:-${SNAPSHOT_PRODUCT}${REGISTRY_SUBTYPE}-${SNAPSHOT_DEPLOYMENT_UNIT}-${SNAPSHOT_CODE_COMMIT}}"
    else
        SNAPSHOT_REPO="${SNAPSHOT_REPO:-${SNAPSHOT_PRODUCT}${REGISTRY_SUBTYPE}-${SNAPSHOT_CODE_COMMIT}}"
    fi
fi
